    ...
    app.post('/api/post/login', (request, response) => {
        ...
        utils.emailOrUsernameExists(username, isUsernameEmail, "*").then(async (result) => {
            ...
            for (const element of result) {
                ...
                const isPasswordCorrect = await bcrypt.compare(password, element.user_password_hash);

                // if password is correct, return user data
                if (isPasswordCorrect) {
                    // store data in session
                    await utils.storeDataInSession(request, element.user_id, element.user_name, element.user_tag, element.user_email, element.user_permissions, element.user_avatar_url);

                    return response.send({ status: 1, message: 'Login successful' });
                }
            }
